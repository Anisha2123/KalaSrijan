      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'14px 2px 0',gap:12}}>
          <div style={{
            ...
            fontSize:'1.35rem',fontWeight:400,color:'rgba(255,230,212,.88)',lineHeight:1.2,
          }}>{image.title}</div>
          <div style={{
            fontFamily:"'Cormorant Garamond',serif",
            fontSize:'1rem',fontWeight:300,color:'rgba(255,198,157,.40)',whiteSpace:'nowrap',
          }}>{idx+1} / {total}</div>
        </div>
    ...
    >
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{
            fontFamily:"'Cormorant Garamond',serif",
            fontSize:'1.07rem',fontWeight:400,color:'#fff',lineHeight:1.2,marginBottom:9,
          }}>{img.title}</p>
          ...
          return (
            ...
            >
              ...
              <div className="st-ov" style={{
                ...
                <span style={{
                  background:meta.color,color:'#fff',
                  fontSize:'.60rem',fontWeight:700,letterSpacing:'.09em',
      ...
      <div style={{ background:'#fff8f4', fontFamily:"'DM Sans',sans-serif", position:'relative', overflow:'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            background:'radial-gradient(ellipse 65% 55% at 85% 20%,rgba(205,44,88,.15) 0%,transparent 60%), radial-gradient(ellipse 50% 42% at 12% 80%,rgba(255,198,157,.07) 0%,transparent 60%)',
          }} />
          {/* petals */}
          {[
          ...
          ].map(([size,top,left,dur,delay,op],i) => (
            <div key={i} style={{
              ...
              animationDuration:dur as string, animationDelay:delay as string,
              animation:`petalFlt ${dur} linear ${delay} infinite`,
            }} />
          ))}

          <div style={{ position:'relative', zIndex:1, maxWidth:680, margin:'0 auto' }}>
            <div style={{ display:'inline-flex', alignItems:'center', gap:12, fontSize:11, letterSpacing:'.20em', textTransform:'uppercase', color:'#FFC69D', fontWeight:500, marginBottom:20, animation:'pgFadeUp .85s ease both' }}>
              <div style={{ width:40, height:1, background:'linear-gradient(90deg,#FFC69D,transparent)' }} />
              Studio Gallery
              <div style={{ width:40, height:1, background:'linear-gradient(270deg,#FFC69D,transparent)' }} />
            </div>
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(2.8rem,6vw,5.6rem)', fontWeight:300, lineHeight:1.05, color:'#fff', marginBottom:16, animation:'pgFadeUp .9s ease .12s both' }}>
              A window into<br /><em style={{ fontStyle:'italic', color:'#FFC69D' }}>Kalasrijan</em>
            </h1>
            <p style={{ fontSize:'1rem', color:'rgba(255,230,212,.55)', fontWeight:300, lineHeight:1.76, maxWidth:460, margin:'0 auto', animation:'pgFadeUp .9s ease .24s both' }}>
        ...
        {/* ── MOVING STRIPS ── */}
        <div style={{ padding:'48px 0', background:'#fff8f4', display:'flex', flexDirection:'column', gap:14, position:'relative', zIndex:1 }}>
          <ScrollStrip imgs={strip1} reverse={false} onOpen={openLb} />
          <ScrollStrip imgs={strip2} reverse={true}  onOpen={openLb} />
        </div>

        {/* ── DIVIDER ── */}
        <div style={{ display:'flex', alignItems:'center', gap:20, maxWidth:1260, margin:'0 auto', padding:'0 36px 44px', position:'relative', zIndex:1 }}>
          <div style={{ flex:1, height:1, background:'linear-gradient(90deg,rgba(205,44,88,.18),transparent)' }} />
          <span style={{ fontSize:'.70rem', letterSpacing:'.16em', textTransform:'uppercase', color:'#7a4a55', fontWeight:500, whiteSpace:'nowrap' }}>
        ...
        {/* ── FILTERS ── */}
        <div ref={headRef} style={{ display:'flex', flexWrap:'wrap', gap:9, justifyContent:'center', maxWidth:1260, margin:'0 auto', padding:'0 36px 40px', position:'relative', zIndex:1,
          opacity: headIn ? 1 : 0, transform: headIn ? 'translateY(0)' : 'translateY(18px)', transition:'opacity .8s ease,transform .8s ease'
        }}>
          {ALL_CATS.map(cat => {
            ...
            return (
              <button key={cat} onClick={() => setActiveFilter(cat)}
                ...
                onMouseEnter={e => { if (!isActive) { (e.currentTarget as HTMLElement).style.background = 'rgba(205,44,88,.07)'; (e.currentTarget as HTMLElement).style.borderColor = '#E06B80'; (e.currentTarget as HTMLElement).style.color = '#CD2C58'; } }}
                onMouseLeave={e => { if (!isActive) { (e.currentTarget as HTMLElement).style.background = 'transparent'; (e.currentTarget as HTMLElement).style.borderColor = 'rgba(205,44,88,.22)'; (e.currentTarget as HTMLElement).style.color = '#7a4a55'; } }}
              >
        ...
        {/* ── MASONRY ── */}
        <div style={{ columns:4, columnGap:14, maxWidth:1260, margin:'0 auto', padding:'0 36px 48px', position:'relative', zIndex:1 }}>
          {shown.map((img, i) => (
            <MasonryCard
              key={img.id}
              img={img}
        ...
        {showCount < filtered.length && (
          <div style={{ textAlign:'center', paddingBottom:80, position:'relative', zIndex:1 }}>
            <button
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background='rgba(205,44,88,.07)'; (e.currentTarget as HTMLElement).style.transform='translateY(-2px)'; }}
